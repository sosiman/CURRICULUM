...
// Cargar modelos GLB
function loadModels() {
    // Modelos del hero principal con nuevos contenedores
    createModelViewer('azure-3d-model', 'models/AZURE3D.glb');
    createModelViewer('cisco-3d-model', 'models/cisco.glb');
    createNetworkingModelViewer('networking-3d-model', 'models/networkin_basics.glb');
    ...
    // El loader ahora usa badge de Credly, no necesita animación 3D
    console.log('✅ Loader configurado con badge de Credly');
}
...
// Crear visor de modelo networking con luces azules (conservando materiales originales)
function createNetworkingModelViewer(containerId, modelPath) {
    const container = document.getElementById(containerId);
    if (!container) return;
    ...
    // Luces mejoradas para mayor claridad y brillo
    const ambientLight = new THREE.AmbientLight(0xffffff, 1.0); // Aumentar luz ambiental
    scene.add(ambientLight);

    const mainLight = new THREE.DirectionalLight(0xffffff, 1.3); // Luz principal más intensa
    mainLight.position.set(5, 10, 7.5);
    scene.add(mainLight);

    const backLight = new THREE.DirectionalLight(0xffffff, 0.8); // Luz de relleno más fuerte
    backLight.position.set(-5, -10, -5);
    scene.add(backLight);
    
    // Luces adicionales para eliminar sombras oscuras
    const topLight = new THREE.DirectionalLight(0xffffff, 0.6);
    topLight.position.set(0, 10, 0);
    scene.add(topLight);
    
    const leftLight = new THREE.PointLight(0xffffff, 0.5);
    leftLight.position.set(-8, 0, 5);
    scene.add(leftLight);
    
    const rightLight = new THREE.PointLight(0xffffff, 0.5);
    rightLight.position.set(8, 0, 5);
    scene.add(rightLight);
    
    // Una luz azul suave adicional para el tinte
    const blueLight = new THREE.PointLight(0x3498db, 0.2); // Reducir intensidad azul
    ...
    loader.load(
        ...
        function(gltf) {
            ...
            // Mantener materiales originales pero hacerlos más claros y brillantes
            model.traverse(function(child) {
                if (child.isMesh && child.material) {
                    // Ajustar todos los materiales para mayor claridad
                    if (child.material.color) {
                        const originalColor = child.material.color.clone();
                        const brightness = originalColor.r + originalColor.g + originalColor.b;
                        
                        // Hacer todos los materiales más brillantes
                        if (brightness < 0.8) {
                            // Aumentar el brillo general
                            child.material.color.multiplyScalar(1.5);
                            
                            // Si es muy oscuro, agregar tinte azul
                            if (brightness < 0.3) {
                                child.material.color.lerp(new THREE.Color(0x3498db), 0.2);
                            }
                        }
                    }
                    
                    // Ajustar propiedades del material para mayor claridad
            ...
            console.log(`✅ ${modelPath} cargado exitosamente conservando materiales originales`);
        },
        undefined,
        function(error) {
            console.error(`❌ Error cargando ${modelPath}:`, error);
...
// Función eliminada: createDefaultLoaderAnimation() - Ahora usamos badge de Credly
